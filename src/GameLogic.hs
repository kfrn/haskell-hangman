...
inputToGuess [guess]
  | Char.isLetter guess = Just guess -- checks for 1 alphabetic character
inputToGuess _ = Nothing

letterInString :: Char -> String -> Bool
letterInString letter string =
  Char.toLower letter `elem` map Char.toLower string

wrongGuesses :: GameState -> [Char]
wrongGuesses (GameState guesses word _) =
  filter (\letter -> not (letterInString letter word)) guesses
...
recordGuess state guess = state {guessedLetters = newGuesses}
  where
    newGuesses = guessedLetters state ++ [guess]
...
displayHangmanWord (GameState guesses word _) = map displayedLetter word
  where
    displayedLetter letter =
      if letterInString letter guesses
        then letter
        else '_'
...
guessLetter state guess
  | letterInString guess (guessedLetters state) = AlreadyGuessed
guessLetter state guess
  | Char.isLetter guess =
    if gameWon newState
  ...
  where
    newState = recordGuess state guess
